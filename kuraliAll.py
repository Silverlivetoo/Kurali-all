    ...
    try:
        if fmt == 'deb':
            if has_cmd('dpkg-deb'):
                ...
                run_cmd(['ar', 'x', str(filepath)], cwd=tmp)
            ...
            with tarfile.open(str(filepath), 'r:gz') as tf: tf.extractall(dest)
...
def cmd_run(filepath):
    ...
    try:
        ...
        if ch.strip().lower() == 'shell':
            ...
            target_exe = exes[int(ch)-1]
            info(f"运行: {target_exe.name}")
            # 使用 Popen 返回进程对象，显示 PID 并提示如何关闭
            proc = subprocess.Popen([str(target_exe)], env=env)
            print(f"\n{C.GREEN}✓ 进程已启动 (PID: {proc.pid}){C.RESET}")
            print(f"  {C.Y}关闭方式:{C.RESET} kill {proc.pid}  或  pkill -f '{target_exe.name}'")
            print(f"  {C.Y}或直接关闭此终端窗口{C.RESET}")
            print(f"  {C.Y}注意: 关闭后 RAM 中的所有数据将被清除{C.RESET}\n")
            # 等待进程结束
            try:
                proc.wait()
...
# ─── 桌面图标 ───
def install_desktop_entry_py(name, display, exec_path, icon=None):
    """创建桌面启动器"""
    ddir = Path.home() / ".local/share/applications"
    idir = Path.home() / ".local/share/icons/hicolor/256x256/apps"
    ...
    if icon and Path(icon).exists():
        icon_dest = idir / f"kurali-{name}.png"
        try:
            shutil.copy2(icon, icon_dest)
        except Exception:
            icon_dest = ""

    df = ddir / f"kurali-{name}.desktop"
...
StartupNotify=true
"""
    if icon_dest and Path(icon_dest).exists():
        content += f"Icon={icon_dest}\n"
    else:
        content += "Icon=application-x-executable\n"

    df.write_text(content)
    # 刷新桌面数据库
    if has_cmd('update-desktop-database'):
        run_cmd(['update-desktop-database', str(ddir)])
    if has_cmd('xdg-desktop-menu'):
        run_cmd(['xdg-desktop-menu', 'forceupdate'])
    ok(f"桌面条目: {name}")

def remove_desktop_entry_py(name):
    """移除桌面启动器"""
    df = Path.home() / f".local/share/applications/kurali-{name}.desktop"
    df.unlink(missing_ok=True)
    idir = Path.home() / ".local/share/icons/hicolor/256x256/apps"
    (idir / f"kurali-{name}.png").unlink(missing_ok=True)
...
def cmd_install(filepath, pkg_name=None, system_mode=False):
    ...
    if pkg_name is None:
        if fmt == 'apk':
            ...
            try:
                ...
                with tarfile.open(str(filepath), 'r:gz') as tf:
                    try:
                        member = tf.getmember('.PKGINFO')
                        tf.extract(member, tmp_a)
                    except KeyError:
                        pass
    ...
    if desktop_ex:
        # 尝试查找图标
        icon_file = None
        for idir in ['usr/share/pixmaps', 'usr/share/icons/hicolor/256x256/apps',
                     'usr/share/icons/hicolor/128x128/apps', 'usr/share/icons/hicolor/64x64/apps']:
            ipath = extract_dir / idir
            if ipath.exists():
                for f in ipath.rglob('*'):
                    if f.suffix in ['.png', '.svg', '.ico']:
                        icon_file = f
                        break
            if icon_file:
                break
        # 安装桌面图标（即使没有图标也创建）
        install_desktop_entry_py(pkg_name, pkg_name, str(desktop_ex), str(icon_file) if icon_file else None)
...
def cmd_deps(target=None):
    if target is None:
        ...
        for lib in ['libc.so','libm.so','libdl.so','libpthread.so','libz.so','libssl.so']:
            # 方法1: ldconfig（匹配库名前缀，如 libc => libc.so.6）
            r = run_cmd(['ldconfig','-p'])
            found = bool(r and r.returncode == 0 and lib.split('.')[0] in r.stdout)
            # 方法2: 常见库路径回退
            if not found:
...
def cmd_pack(filepath, output=None):
    ...
    if fmt == 'deb':
        ...
        r = run_cmd(['dpkg-deb', '-f', str(filepath), 'Version'])
        pkg_version = r.stdout.strip() if r and r.stdout.strip() else "unknown"
    elif fmt == 'rpm':
        r = run_cmd(['rpm', '-qp', '--qf', '%{NAME}', str(filepath)])
        pkg_name = r.stdout.strip().lower() if r and r.stdout.strip() else filepath.stem.split('-')[0].lower()
        ...
        pkg_name = filepath.stem.replace('.pkg.tar.zst', '').replace('.pkg.tar.xz', '').replace('.pkg.tar.gz', '').lower()
        pkg_version = "unknown"
    elif fmt == 'apk':
        pkg_name = filepath.stem.split('-')[0].lower()
        pkg_version = "unknown"
    ...
    try:
        ...
        with tarfile.open(output, "r:gz") as tar:
            ...
            if ".kurali/manifest.json" not in members:
                warn("警告: 打包内容可能不完整")

        file_count = len(files_list)
        out_size = Path(output).stat().st_size / 1024
        ok(f"打包完成: {output} ({out_size:.1f}KB, {file_count} 个文件)")
        info(f"来源: {filepath.name} [{fmt}] → .kurali")
        print(f"  {C.CY}直接运行:{C.RESET} kurali-py --ram {output}")
        print(f"  {C.CY}安装:{C.RESET} kurali-py i {output}")
    finally:
        shutil.rmtree(tmp, ignore_errors=True)

# ─── 帮助 ───
HELP = """KuraliAll v2.2.0 — Python 版全能包管理器
