#!/usr/bin/env python3
"""
KuraliAll v2.1 — 终端 GUI 版 (curses)
        ...
        if key == ord('q') or key == ord('Q'):
            ...
            if action == "quit":
                ...
                if filepath:
                    ...
                    stdscr.refresh()
                    r = run_cmd(["python3", __file__, "i", filepath, "-y"])
                    if r and r.returncode == 0:
                        status_msg = f"安装成功: {filepath}"
                        status_color = 1
                    else:
                        status_msg = f"安装失败: {r.stderr[:50] if r else '未知错误'}"
                        status_color = 2
                    packages = get_packages()
            elif action == "remove":
                if not packages:
                ...
                curses.curs_set(0)
                if name:
                    r = run_cmd(["python3", __file__, "r", name, "-y"])
                    if r and r.returncode == 0:
                ...
                if filepath:
                    ...
                    print(f"{'='*50}\n")
                    r = subprocess.run(["python3", __file__, "run", filepath])
                    input("\n按回车返回 GUI...")
                    stdscr.refresh()
                    stdscr.clear()
            elif action == "help":
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] in ('i','install','r','remove','run','exec'):
        # CLI 模式支持从 GUI 调用
        sys.argv[0] = str(Path(__file__).parent / "kuraliAll.py")
        import importlib.util
        spec = importlib.util.spec_from_file_location("kuraliAll",
            str(Path(__file__).parent / "kuraliAll.py"))
        mod = importlib.util.module_from_spec(spec)
        sys.modules["kuraliAll"] = mod
