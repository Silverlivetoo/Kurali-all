...
setup_ram() {
    ...
    # 备用：检查 /tmp 是否是 tmpfs
    if [[ -z "$base" ]]; then
        local fstype; fstype=$(df -T /tmp 2>/dev/null | awk 'NR==2{print $2}')
        if [[ "$fstype" == "tmpfs" || "$fstype" == "devtmpfs" ]]; then
            base="/tmp"
        else
            base="/tmp"
            warn "/tmp 非 tmpfs (${fstype:-unknown}), 安全性稍低"
        fi
    fi
    [[ -z "$base" ]] && die "无可用临时目录"
...
run_in_ram() {
    ...
    if [[ "$ch" == "shell" ]]; then
        ...
        (cd "$RAM_TMPDIR" && exec bash --norc)
    elif [[ "$ch" =~ ^[0-9]+$ ]] && [[ "$ch" -ge 1 && "$ch" -le "${#exe_arr[@]}" ]]; then
        "${exe_arr[$((ch-1))]}"
    else
        die "无效选择"
    fi
