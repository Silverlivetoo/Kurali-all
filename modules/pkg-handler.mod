...
get_pkg_name() {
    ...
    case "$format" in
        deb)
            has_cmd dpkg-deb && name=$(dpkg-deb -f "$file" Package 2>/dev/null) || \
            { has_cmd ar && name=$(basename "$file" .deb); }
            ;;
        rpm)
            has_cmd rpm && name=$(rpm -qp --qf "%{NAME}" "$file" 2>/dev/null) || \
...
EOF
    ...
    # 合并统计到 manifest.json
    sed -i '/}$/i\  "files": '${file_count}',' "${meta}/manifest.json"
    sed -i '/}$/i\  "size": "'${total_size}'"' "${meta}/manifest.json"

    # 打包
    tar czf "$output" -C "$tmp" .kurali rootfs 2>/dev/null
    [[ -d "${scripts}" && -n "$(ls -A "$scripts" 2>/dev/null)" ]] && \
