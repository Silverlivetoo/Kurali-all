        ...
        if desktop_ex:
            ...
            try:
                ...
                if icon_file and icon_file.exists():
                    icon_dest = idir / f"kurali-{pkg_name}.png"
                    shutil.copy2(icon_file, icon_dest)

                df = ddir / f"kurali-{pkg_name}.desktop"
                content = f"""[Desktop Entry]
Name={pkg_name}
Comment=Installed via KuraliAll: {pkg_name}
Exec={desktop_ex}
Terminal=false
Type=Application
Categories=Utility;
...
"""
                ...
                add_log("INFO", f"已创建桌面图标: {pkg_name}")
            except Exception as e:
                add_log("WARN", f"创建桌面图标失败: {e}")
...
def do_deps_check(target=None):
    ...
    if target is None:
        ...
        for lib in ['libc.so','libm.so','libdl.so','libpthread.so','libz.so','libssl.so']:
            found = False
            # 使用库名前缀匹配（libc => libc.so.6）
            lib_prefix = lib.split('.')[0]
            if has_cmd('ldconfig'):
                r2 = run_cmd(['ldconfig','-p'])
                if r2 and r2.returncode == 0 and lib_prefix in r2.stdout: found = True
            if not found:
                for d in ['/lib','/lib64','/usr/lib','/usr/lib64','/usr/lib/x86_64-linux-gnu','/lib/x86_64-linux-gnu']:
                    if Path(d).is_dir():
                        try:
                            for f in os.listdir(d):
                                if f.startswith(lib_prefix): found = True; break
                        except: pass
                    if found: break
